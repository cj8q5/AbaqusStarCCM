''' This Python file runs within a compiled version of AbaqusStarCCM.cs. This script is used
		in Abaqus and creates the solid geometry as well as the fluid mesh. The fluid
		mesh will be imported into Star-CCM+ using a .inp file generated by this script. 
...
#-----------------------------------------------------------------------------------------------------------
'''
	This method creates a flat plate for an FSI simulation
'''
def createFlatPlate(geometryParameters):
	...
	seedMethod = parameters['seedMethod']
	plateThick_1 = parameters['solidThick_1']
	plateThick_2 = parameters['solidThick_2']
	plateLength_1 = parameters['solidLength_1']
	plateLength_2 = parameters['solidLength_2']
	plateWidth_1 = parameters['solidWidth_1']
	plateWidth_2 = parameters['solidWidth_2']
	clampedWidth_1 = parameters['clampedWidth_1']
	clampedWidth_2 = parameters['clampedWidth_2']
	...
	abqSolver = parameters['abqSolver']
	...
	entirePlateGeometry = plateCells.getByBoundingBox(
	                          ...
	                          xMax = plateWidth*2, yMax = plateLength*2, zMax = 1)
	...
	if seedMethod == 'size':		   
		plate.seedEdgeByBias(biasMethod=DOUBLE, endEdges=plateLengthEdges, minSize=plateLength_2,
			maxSize=plateLength_1, constraint=FINER)
		
		plate.seedEdgeByBias(biasMethod=DOUBLE, endEdges=plateWidthEdges, minSize=plateWidth_2,
			maxSize=plateWidth_1, constraint=FINER)
		if elemType == 'SC8R':
			plate.seedEdgeByNumber(edges=plateThicknessEdges, number=1, constraint=FINER)
		else:
			plate.seedEdgeByBias(biasMethod=DOUBLE, endEdges=plateThicknessEdges, minSize=plateThick_2,
				maxSize=plateThick_1, constraint=FINER)
		
		plate.seedEdgeByBias(biasMethod=DOUBLE, endEdges=clampedWidthEdges, minSize=clampedWidth_2,
			maxSize=clampedWidth_1, constraint=FINER)
		   
	if seedMethod == 'number':		
		plate.seedEdgeByBias(biasMethod=DOUBLE, endEdges=plateLengthEdges, ratio=plateLength_2,
			number=plateLength_1, constraint=FINER)
		
		plate.seedEdgeByBias(biasMethod=DOUBLE, endEdges=plateWidthEdges, ratio=plateWidth_2,
			number=plateWidth_1, constraint=FINER)
			
		if elemType == 'SC8R':
			plate.seedEdgeByNumber(edges=plateThicknessEdges, number=1, constraint=FINER)
		else:
			plate.seedEdgeByBias(biasMethod=DOUBLE, endEdges=plateThicknessEdges, ratio=plateThick_2,
				number=plateThick_1, constraint=FINER)
		
		plate.seedEdgeByBias(biasMethod=DOUBLE, endEdges=clampedWidthEdges, ratio=clampedWidth_2,
			number=clampedWidth_1, constraint=FINER)
	...
	appendInputFile(starVersion, abqSolver, couplingScheme, BC, timeStep, maxSimTime, minTimeStep)
...
def createFlatFluid(parameters):
	...
	seedMethod = parameters['seedMethod']
	flPlLen_1 = parameters['flPlLen_1']
	flPlLen_2 = parameters['flPlLen_2']
	flPlWidth_1 = parameters['flPlWidth_1']
	flPlWidth_2 = parameters['flPlWidth_2']
	flInlet_1 = parameters['flInlet_1']
	flInlet_2 = parameters['flInlet_2']
	flOutlet_1 = parameters['flOutlet_1']
	flOutlet_2 = parameters['flOutlet_2']
	flSmChHeight_1 = parameters['flSmChHeight_1']
	flSmChHeight_2 = parameters['flSmChHeight_2']
	smChWallCellHgt = parameters['flSmChWallCellHgt']
	flLgChHeight_1 = parameters['flLgChHeight_1']
	flLgChHeight_2 = parameters['flLgChHeight_2']
	lgChWallCellHgt = parameters['flLgChWallCellHgt']
	flPlHeight_1 = parameters['flPlHeight_1']
	flPlHeight_2 = parameters['flPlHeight_2']
	biasDirection = parameters['chBiasDirection']
	lowOrHighyPlus = parameters['lowOrHighyPlus']
	...
	entireFluidGeometry = fluidCells.getByBoundingBox(
								xMin = -1, yMin = -outletPlLength-1, zMin = -1,
								xMax = plateWidth*2, yMax = plateLength+inletPlLength+1, zMax = 1)
	fluid.Set(cells = entireFluidGeometry, name = 'EntireFluidGeometry')

	# Partitioning the fluid for meshing
	if lowOrHighyPlus == 'High':
		numOfPlanes = (numOfPlates*4)+6
	elif lowOrHighyPlus == 'Low':
		numOfPlanes = (numOfPlates*2)+4
	for i in range(2, numOfPlanes):
	...
	for i in range(0, numOfPlates+1):
		...
		for k in range(0, 2):
			for j in range(0, len(yPos)):
				...
				if lowOrHighyPlus == 'High':
					flowPlateEdges.append(fluidEdges.findAt(
							((halfPlateWidth, yPos[j], -lgChHeight + lgplateSpacing*i + ch),) ))
					flowPlateEdges.append(fluidEdges.findAt( 
							((halfPlateWidth, yPos[j], smplateSpacing*i - ch),) ))
	fluid.Set(edges=flowPlateEdges, name='FluidWidth')

	# Creating the set of faces for the inlet and outlet
		...
		for j in range(0, numOfPlates+1):
			...
			if lowOrHighyPlus == 'High':
				inletFaces.append(fluidFaces.findAt( 
									((halfPlateWidth, pos[(i*2)+1], -lgChHeight + lgplateSpacing*j + ch*0.5),) ))
				inletFaces.append(fluidFaces.findAt( 
									((halfPlateWidth, pos[(i*2)+1], smplateSpacing*j - ch*0.5),) ))
			inletFaces.append(fluidFaces.findAt( 
								((halfPlateWidth, pos[(i*2)+1], -lgChHeight*0.5 + lgplateSpacing*j),) ))
			inletFaces.append(fluidFaces.findAt( 
	...
	if seedMethod == 'number':
		fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidPlateLengthEdges, ratio=flPlLen_2,
		   number=flPlLen_1, constraint=FINER)

		fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidWidthEdges, ratio=flPlWidth_2,
		   number=flPlWidth_1, constraint=FINER)

		fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidOutletEdges, ratio=flOutlet_2,
		   number=flOutlet_1, constraint=FINER)

		fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidInletEdges, ratio=flInlet_2,
		   number=flInlet_1, constraint=FINER)

		if biasDirection == 'Center':
			fluid.seedEdgeByBias(biasMethod=DOUBLE, centerEdges=fluidSmallChEdges, ratio=flSmChHeight_2,
    			number=flSmChHeight_1, constraint=FINER)

			fluid.seedEdgeByBias(biasMethod=DOUBLE, centerEdges=fluidLargeChEdges, ratio=flLgChHeight_2,
    			number=flLgChHeight_1, constraint=FINER)

			fluid.seedEdgeByBias(biasMethod=DOUBLE, centerEdges=fluidPlateEdges, ratio=flPlHeight_2,
				number=flPlHeight_1, constraint=FINER)
		elif biasDirection == 'End':
			fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidSmallChEdges, ratio=flSmChHeight_2,
    			number=flSmChHeight_1, constraint=FINER)

			fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidLargeChEdges, ratio=flLgChHeight_2,
    			number=flLgChHeight_1, constraint=FINER)

			fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidPlateEdges, ratio=flPlHeight_2,
    			number=flPlHeight_1, constraint=FINER)

	if seedMethod == 'size':
		fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidPlateLengthEdges, minSize=flPlLen_1,
		   maxSize=flPlLen_2, constraint=FINER)

		fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidWidthEdges, minSize=flPlWidth_1,
		   maxSize=flPlWidth_2, constraint=FINER)

		fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidOutletEdges, minSize=flOutlet_1,
		   maxSize=flOutlet_2, constraint=FINER)

		fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidInletEdges, minSize=flInlet_1,
		   maxSize=flInlet_2, constraint=FINER)

		if biasDirection == 'Center':
			fluid.seedEdgeByBias(biasMethod=DOUBLE, centerEdges=fluidSmallChEdges, minSize=flSmChHeight_1,
    			maxSize=flSmChHeight_2, constraint=FINER)

			fluid.seedEdgeByBias(biasMethod=DOUBLE, centerEdges=fluidLargeChEdges, minSize=flLgChHeight_1,
    			maxSize=flLgChHeight_2, constraint=FINER)

			fluid.seedEdgeByBias(biasMethod=DOUBLE, centerEdges=fluidPlateEdges, minSize=flPlHeight_1,
				maxSize=flPlHeight_2, constraint=FINER)
		elif biasDirection == 'End':
			fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidSmallChEdges, minSize=flSmChHeight_1,
    			maxSize=flSmChHeight_2, constraint=FINER)

			fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidLargeChEdges, minSize=flLgChHeight_1,
    			maxSize=flLgChHeight_2, constraint=FINER)

			fluid.seedEdgeByBias(biasMethod=DOUBLE, endEdges=fluidPlateEdges, minSize=flPlHeight_1,
    			maxSize=flPlHeight_2, constraint=FINER)
...
def appendInputFile(starVersion, abqSolver, couplingScheme, BC, timeStep, maxSimTime, minTimeStep):
	# Appending the written input file for explicit/implicit FSI coupling
...
def curved_sketch(part, modelName, r_in, emptyTheta, plateThickness, plateOrFluid):
	...
	datumParts = part.datums
	t = part.MakeSketchTransform(sketchPlane=datumParts[1], origin=(0.0, 0.0, 0.0), 
								sketchOrientation=RIGHT, sketchPlaneSide=SIDE1, sketchUpEdge=datumParts[2])
...
parameters = fileReader('InputFile.txt')
...
elif parameters['plateGeometry'] == 'Curved':
