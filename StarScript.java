/** This Java file runs within a compiled version of AbaqusStarCCM.cs. It creates the Star-CCM+ file using 
 		an Abaqus .inp file which contains a mesh generated in Abaqus. The .inp file should be created using
		using the Abaqus.py script.
...
public class StarScript extends StarMacro 
{
	public void execute() 
	{
		...
		String CFDorFSI = reader.getStringData("CFDOrFSI");
		...
		// Grabbing boundary conditions and fluid properties
		double avgChVel = reader.getDoubleData("avgChVelocity");
		double fluidDensity = reader.getDoubleData("fluidDensity");
		double fluidViscosity = reader.getDoubleData("fluidViscosity");
		...
		// Setting the density and viscosity of the fluid
		physics.setFluidDensity(fluidPhysics, fluidDensity);
		physics.setFluidViscosity(fluidPhysics, fluidViscosity);
		...
		String parametricSwitch = reader.getStringData("parametricSwitch");
		if (parametricSwitch.equals("true"))
		{
			abaqus.setAbaqusExecutionSettings("Parametric_Study_" + CFDorFSI + "_Model_Abaqus", 
					abaqusInputFilePath, abqExecutableFileName, numAbaqusCPUs);			
		}
		else if(numOfPlates == 1)
		{
			abaqus.setAbaqusExecutionSettings("FSI_" + (int)Math.abs(initialVel[1]) + "_" + plateGeometry + "_" +
					(int)(plateThickness/0.0254*1000) +	"_" + (int)(smChHeight/0.0254*1000) + "_" + (int)(lgChHeight/0.0254*1000) + "_Abaqus", 
					abaqusInputFilePath, abqExecutableFileName, numAbaqusCPUs);
		}
		else
		{
		...
		if(CFDorFSI.equals("CFD"))
		{
			solvers.setSSorFSI(true);
		}
		else if(CFDorFSI.equals("FSI"))
		{
			solvers.setSSorFSI(false);
		}
		
		/**-----------------------------------------------------------------------------------------------------------------------------------------------------
			REPORTS, MONITORS, AND STOPPING CRITERIA NODES */
		...
		// Creating stopping criteria for the wall shear stress on the plate's wall
		...
		// Creating reports, monitors, and a plot for the Wall y+ values
		AreaAverageReport avgWallYPlus = reports.createAverageReport(fsiSurfaces2, "AvgWallYPlus");
		MaxReport maxWallYPlus = reports.createMaxReport(fsiSurfaces2, "MaxWallYPlus");
		MinReport minWallYPlus = reports.createMinReport(fsiSurfaces2, "MinWallYPlus");
		
		PrimitiveFieldFunction wallYPlus = fieldFunctions.getWallYPlusFunction();
		avgWallYPlus.setScalar(wallYPlus);
		maxWallYPlus.setScalar(wallYPlus);
		minWallYPlus.setScalar(wallYPlus);
		
		reports.createMonitorPlot2(new String[] {"MinWallYPlus", "MaxWallYPlus", "AvgWallYPlus"}, "Avg, Max, and Min Wall y+",
				new String[] {"Iteration", "Wall y+ (-)"}, true);
		...
		*/
		...
		if (parametricSwitch.equals("true"))
		{
			saveLocation = currentDirectory + File.separator + "Parametric_Study_" + CFDorFSI + "_Model_.sim";				
		}
		else if(numOfPlates == 1)
		{
			saveLocation = currentDirectory + File.separator + 
					CFDorFSI + "_" + (int)Math.abs(initialVel[1]) + "_" + plateGeometry + '_' +
		...
		else
		{
			saveLocation = currentDirectory + File.separator + 
					CFDorFSI + "_" + (int)Math.abs(initialVel[1]) + "_" + plateGeometry + '_' +
					(int)(plateThickness/0.0254*1000) + "_" + (int)(smChHeight/0.0254*1000) + 
					"_" + numOfPlates + "_PlateStack.sim";
		}
		activeSim.saveState(saveLocation);
		if (reader.getStringData("runStar").equals("yes"))
		{
